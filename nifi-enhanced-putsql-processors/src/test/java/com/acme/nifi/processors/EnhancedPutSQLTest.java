...
public class EnhancedPutSQLTest {

    private TestRunner runner;
    private EnhancedPutSQL processor;
    ...
    public void setUp() {
        ...
        try {
            ...
            runner.setProperty(EnhancedPutSQL.ENABLE_VALIDATION, "true");
            runner.setProperty(EnhancedPutSQL.SOURCE_DBCP_SERVICE, "sourceDbcp");
            runner.setProperty(EnhancedPutSQL.TARGET_DBCP_SERVICE, "targetDbcp");
            runner.setProperty(EnhancedPutSQL.ENGINE_DBCP_SERVICE, "engineDbcp");
            runner.setProperty(EnhancedPutSQL.CONNECTION_POOL, "operationDbcp");
            runner.setProperty(EnhancedPutSQL.OPERATION_ID, "testOpId");
            runner.setProperty(EnhancedPutSQL.VALIDATION_MODE, "STRICT");
            runner.setProperty(EnhancedPutSQL.AUTO_COMMIT, "true");
            runner.setProperty(EnhancedPutSQL.SUPPORT_TRANSACTIONS, "false");
            runner.setProperty(EnhancedPutSQL.BATCH_SIZE, "100");
            runner.setProperty(EnhancedPutSQL.OBTAIN_GENERATED_KEYS, "false");
    ...
    public void testProcessorWithValidSQL() {
        try {
            ...
            when(mockEnvRs.getString("source_db_name")).thenReturn("testdb");
            ...
            when(mockEnvRs.getString("target_db_name")).thenReturn("testdb");
            ...
            when(mockOperationConnection.prepareStatement("INSERT INTO test_table (id) VALUES (?)")).thenReturn(mockStmt);
            ...
            // Verify results
            runner.assertTransferCount(EnhancedPutSQL.REL_SUCCESS, 1);
            runner.assertTransferCount(EnhancedPutSQL.REL_FAILURE, 0);
            runner.assertTransferCount(EnhancedPutSQL.REL_RETRY, 0);
            runner.assertTransferCount(EnhancedPutSQL.REL_VALIDATION_FAILED, 0);

            MockFlowFile flowFile = runner.getFlowFilesForRelationship(EnhancedPutSQL.REL_SUCCESS).get(0);
    ...
    public void testInvalidOperationId() {
        try {
            ...
            // Verify results
            runner.assertTransferCount(EnhancedPutSQL.REL_SUCCESS, 0);
            runner.assertTransferCount(EnhancedPutSQL.REL_FAILURE, 1);
            runner.assertTransferCount(EnhancedPutSQL.REL_RETRY, 0);
            runner.assertTransferCount(EnhancedPutSQL.REL_VALIDATION_FAILED, 0);

            MockFlowFile flowFile = runner.getFlowFilesForRelationship(EnhancedPutSQL.REL_FAILURE).get(0);
            flowFile.assertAttributeEquals("validation.passed", "false");
            flowFile.assertAttributeEquals("validation.error", "Environment configuration not found");
        } catch (SQLException e) {
    ...
    public void testValidationFailure() {
        try {
            ...
            runner.assertTransferCount(EnhancedPutSQL.REL_FAILURE, 0);
            runner.assertTransferCount(EnhancedPutSQL.REL_RETRY, 0);
            runner.assertTransferCount(EnhancedPutSQL.REL_VALIDATION_FAILED, 1);

            MockFlowFile flowFile = runner.getFlowFilesForRelationship(EnhancedPutSQL.REL_VALIDATION_FAILED).get(0);
            flowFile.assertAttributeEquals("validation.error", "Controller service mismatch");
        } catch (SQLException e) {
    ...
    public void testValidationFailureWithStatusUpdate() {
        try {
            // Note: FAILURE_ACTION property was removed - validation failures route to validation_failed relationship
            ...
            // Verify results - validation failure should route to validation_failed in STRICT mode
            runner.assertTransferCount(EnhancedPutSQL.REL_SUCCESS, 0);
            runner.assertTransferCount(EnhancedPutSQL.REL_FAILURE, 0);
            runner.assertTransferCount(EnhancedPutSQL.REL_RETRY, 0);
            runner.assertTransferCount(EnhancedPutSQL.REL_VALIDATION_FAILED, 1);
            ...
            fail("Failed to set up mocked database connections in testValidationFailureWithStatusUpdate: " + e.getMessage());
        }
    }
}