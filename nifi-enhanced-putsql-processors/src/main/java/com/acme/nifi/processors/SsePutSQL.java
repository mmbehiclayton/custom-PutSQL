...
public class SsePutSQL extends AbstractSessionFactoryProcessor {
    ...
    public static final PropertyDescriptor ENABLE_VALIDATION = new PropertyDescriptor.Builder()
            ...
            .description(
                    "Enable or disable environment validation. Set to false only for testing or emergency bypass. When false, processor behaves exactly like standard PutSQL.")
            .required(true)
            .defaultValue("false")
    ...
    public static final PropertyDescriptor ENGINE_DBCP_SERVICE = new PropertyDescriptor.Builder()
            ...
            .description(
                    "The DBCP Controller Service for connecting to the SSE Engine database to query environment configurations")
            .required(false)
    ...
    static final PropertyDescriptor CONNECTION_POOL = new PropertyDescriptor.Builder()
            ...
            .description(
                    "Specifies the JDBC Connection Pool to use in order to convert the JSON message to a SQL statement. "
    ...
    static final PropertyDescriptor SQL_STATEMENT = new PropertyDescriptor.Builder()
            ...
            .description(
                    "The SQL statement to execute. The statement can be empty, a constant value, or built from attributes "
    ...
    static final PropertyDescriptor SUPPORT_TRANSACTIONS = new PropertyDescriptor.Builder()
            ...
            .description(
                    "If true, when a FlowFile is consumed by this Processor, the Processor will first check the fragment.identifier and fragment.count attributes of that FlowFile. "
    ...
    static final PropertyDescriptor TRANSACTION_TIMEOUT = new PropertyDescriptor.Builder()
            ...
            .description(
                    "If the <Support Fragmented Transactions> property is set to true, specifies how long to wait for all FlowFiles for a particular fragment.identifier attribute "
    ...
    static final PropertyDescriptor OBTAIN_GENERATED_KEYS = new PropertyDescriptor.Builder()
            ...
            .description(
                    "If true, any key that is automatically generated by the database will be added to the FlowFile that generated it using the sql.generate.key attribute. "
    ...
    // Caffeine caches for environment validation (persistent across batches)
    private final Cache<String, EnvironmentConfig> environmentConfigCache;
    private final Cache<String, String> jdbcUrlCache;

    /**
     * Constructor - initializes Caffeine caches for environment validation
     */
    public SsePutSQL() {
        // Initialize environment config cache: 5 minute expiry, max 1000 entries
        this.environmentConfigCache = Caffeine.newBuilder()
                .expireAfterWrite(5, TimeUnit.MINUTES)
                .maximumSize(1000)
                .build();

        // Initialize JDBC URL cache: 30 minute expiry, max 100 entries
    ...
    private final PartialFunctions.InitConnection<FunctionContext, Connection> initConnection = (c, s, fc, ffs) -> {
        ...
        if (dbcpService == null) {
            throw new ProcessException(
                    "DBCP Service class not available. Ensure nifi-dbcp-service-api is available in NiFi runtime.");
        }
    ...
    private final GroupingFunction groupFragmentedTransaction = (context, session, fc, conn, flowFiles, groups,
            result) -> {
        ...
        for (final FlowFile flowFile : flowFiles) {
            final String sql = context.getProperty(SQL_STATEMENT).isSet()
                    ? context.getProperty(SQL_STATEMENT).evaluateAttributeExpressions(flowFile)
                            .getValue()
    ...
    private final GroupingFunction groupFlowFilesBySQLBatch = (context, session, fc, conn, flowFiles, groups,
            result) -> {
        for (final FlowFile flowFile : flowFiles) {
            final String sql = context.getProperty(SQL_STATEMENT).isSet()
                    ? context.getProperty(SQL_STATEMENT).evaluateAttributeExpressions(flowFile)
            ...
            // Create a new PreparedStatement or reuse the one from the last group if that
            // is the same.
    ...
    final PutGroup.GroupFlowFiles<FunctionContext, Connection, StatementFlowFileEnclosure> groupFlowFiles = (context,
            session, fc, conn, flowFiles, result) -> {
        final List<StatementFlowFileEnclosure> groups = new ArrayList<>();

        // There are three patterns:
        // 1. Support batching: An enclosure has multiple FlowFiles being executed in a
        // batch operation
        // 2. Obtain keys: An enclosure has multiple FlowFiles, and each FlowFile is
        // executed separately
    ...
    final PutGroup.PutFlowFiles<FunctionContext, Connection, StatementFlowFileEnclosure> putFlowFiles = (context,
            session, fc, conn, enclosure, result) -> {
        ...
        if (fc.isSupportBatching()) {
            ...
            for (final FlowFile flowFile : enclosure.getFlowFiles()) {
                ...
                final StatementFlowFileEnclosure targetEnclosure = enclosure instanceof FragmentedEnclosure
                ...
                exceptionHandler.execute(fc, flowFile, input -> {
                    try (final PreparedStatement stmt = targetEnclosure.getNewStatement(conn, fc.obtainKeys)) {
                        ...
                        // attempt to determine the key that was generated, if any. This is not
                        // supported by all
                        // database vendors, so if we cannot determine the generated key (or if the
    ...
    private ExceptionHandler.OnError<FunctionContext, FlowFile> onFlowFileError(final ProcessContext context,
            final ProcessSession session, final RoutingResult result) {
        ExceptionHandler.OnError<FunctionContext, FlowFile> onFlowFileError = createOnError(context, session, result,
        ...
        onFlowFileError = onFlowFileError.andThen((ctx, flowFile, errorTypesResult, exception) -> {
            ...
            switch (errorTypesResult.destination()) {
                case Failure:
                    getLogger().error("Failed to update database for {} due to {}; routing to failure", flowFile,
                            exception, exception);
    ...
    private ExceptionHandler.OnError<RollbackOnFailure, FlowFileGroup> onGroupError(final ProcessContext context,
            final ProcessSession session, final RoutingResult result) {
        ExceptionHandler.OnError<RollbackOnFailure, FlowFileGroup> onGroupError = ExceptionHandler
                .createOnGroupError(context, session, result, REL_FAILURE, REL_RETRY);
        ...
        onGroupError = onGroupError.andThen((ctx, flowFileGroup, errorTypesResult, exception) -> {
            switch (errorTypesResult.destination()) {
                case Failure:
                    ...
                    result.getRoutedFlowFiles().put(REL_FAILURE, addErrorAttributesToFlowFilesInGroup(session,
                            flowFilesToFailure, flowFileGroup.getFlowFiles(), exception));
                    break;
                case Retry:
                    List<FlowFile> flowFilesToRetry = getFlowFilesOnRelationship(result, REL_RETRY);
                    result.getRoutedFlowFiles().put(REL_RETRY, addErrorAttributesToFlowFilesInGroup(session,
                            flowFilesToRetry, flowFileGroup.getFlowFiles(), exception));
                    break;
            }
        });
    ...
    private List<FlowFile> addErrorAttributesToFlowFilesInGroup(ProcessSession session,
            List<FlowFile> flowFilesOnRelationship, List<FlowFile> flowFilesInGroup, Exception exception) {
    ...
    private ExceptionHandler.OnError<FunctionContext, StatementFlowFileEnclosure> onBatchUpdateError(
        ...
        return RollbackOnFailure.createOnError((c, enclosure, r, e) -> {
            ...
            // If rollbackOnFailure is enabled, the error will be thrown as ProcessException
            // instead.
            if (e instanceof BatchUpdateException && !c.isRollbackOnFailure()) {

                // If we get a BatchUpdateException, then we want to determine which FlowFile
                // caused the failure,
                // and route that FlowFile to failure while routing those that finished
                // processing to success and those
                ...
                // In the presence of a BatchUpdateException, the driver has the option of
                // either stopping when an error
                // occurs, or continuing. If it continues, then it must account for all
                // statements in the batch and for
                // those that fail return a Statement.EXECUTE_FAILED for the number of rows
                // updated.
                // So we will iterate over all of the update counts returned. If any is equal to
                // Statement.EXECUTE_FAILED,
                // we will route the corresponding FlowFile to failure. Otherwise, the FlowFile
                // will go to success
                ...
                if (failureCount == 0) {
                    // if no failures found, the driver decided not to execute the statements after
                    // the
                ...
                if (updateCounts.length < batchFlowFiles.size()) {
                    final List<FlowFile> unexecuted = batchFlowFiles.subList(updateCounts.length + 1,
                            batchFlowFiles.size());
                ...
                getLogger().error(
                        ...
                        e, failureCount, successCount, retryCount, e);
            ...
            ExceptionHandler.OnError<RollbackOnFailure, FlowFileGroup> onGroupError = onGroupError(context, session,
                    result);
    ...
    public void constructProcess() {
        ...
        process.onCompleted((c, s, fc, conn) -> {
            try {
                // Only call commit() if auto-commit is false, per the JDBC spec (see
                // java.sql.Connection)
        ...
        process.onFailed((c, s, fc, conn, e) -> {
            try {
                // Only call rollback() if auto-commit is false, per the JDBC spec (see
                // java.sql.Connection)
                if (!conn.getAutoCommit()) {
                    conn.rollback();
                }
                ...
                // ProcessSession will be rollback by the thrown Exception so don't have to do
                // anything here.
    ...
    public void onTrigger(ProcessContext context, ProcessSessionFactory sessionFactory) throws ProcessException {
        // Standard PutSQL execution using RollbackOnFailure pattern
        // Validation is now handled inside pollFlowFiles to avoid double-consumption of
        // FlowFiles
    ...
    /**
     * Pulls a batch of FlowFiles from the incoming queues. If no FlowFiles are
     * available, returns <code>null</code>.
     * Otherwise, a List of FlowFiles will be returned.
     * <p>
     * If all FlowFiles pulled are not eligible to be processed, the FlowFiles will
     * be penalized and transferred back
    ...
    private FlowFilePoll pollFlowFiles(final ProcessContext context, final ProcessSession session,
        ...
        final List<FlowFile> candidateFlowFiles;
        // The ConnectionPool service has a FlowFileFilter when, and only when it is a
        ...
        if (dbcpServiceFlowFileFilter != null
            ...
            candidateFlowFiles = List.of();
        } else {
            candidateFlowFiles = selectedFlowFiles;
        }

        if (candidateFlowFiles.isEmpty()) {
            return null;
        }

        final List<FlowFile> validFlowFiles;

        List<FlowFile> processedFlowFiles = new ArrayList<>();
        if (context.getProperty(ENABLE_VALIDATION).asBoolean()) {
            // Pre-fetch services
            ...
            // Batch-local caches (secondary layer for performance within this batch)
            Map<String, EnvironmentConfig> batchEnvironmentConfigCache = new HashMap<>();
            Map<Object, String> batchJdbcUrlCache = new HashMap<>();

            for (FlowFile flowFile : candidateFlowFiles) {
                // ... validation logic ...
                // If failed, route and continue (don't add to processedFlowFiles)
                // If success, update attributes, add NEW flowfile to processedFlowFiles
                ...
                String operationId = context.getProperty(OPERATION_ID).evaluateAttributeExpressions(flowFile)
                        .getValue();
                ...
                try {
                    ValidationResultData validationResult = performEnvironmentValidation(context, flowFile, engineDbcp,
                            sourceDbcp, targetDbcp, operationId, batchEnvironmentConfigCache, batchJdbcUrlCache);
                    if (!validationResult.isValid()) {
                        ...
                        if ("STRICT".equals(validationMode)) {
                            ...
                            result.routeTo(flowFile, REL_VALIDATION_FAILED);
                            continue;
                        } else {
            ...
            validFlowFiles = processedFlowFiles;
        } else {
            validFlowFiles = candidateFlowFiles;
        }
        ...
        // If we are supporting fragmented transactions, verify that all FlowFiles are
        // correct
        if (fragmentedTransaction) {
            try {
                if (!isFragmentedTransactionReady(validFlowFiles,
    ...
    /**
     * Determines which relationship the given FlowFiles should go to, based on a
     * transaction timing out or
     ...
     *
     * @param flowFiles                the FlowFiles whose relationship is to be
     *                                 determined
     * @param transactionTimeoutMillis the maximum amount of time (in milliseconds)
     *                                 that we should wait
     *                                 for all FlowFiles in a transaction to be
     *                                 present before routing to failure
     ...
     */
    boolean isFragmentedTransactionReady(final List<FlowFile> flowFiles, final Long transactionTimeoutMillis)
        ...
        for (final FlowFile flowFile : flowFiles) {
            ...
            if (fragmentCount == null && flowFiles.size() == 1) {
                ...
                throw illegal.apply(
                        "Cannot process %s because there are %d FlowFiles with the same fragment.identifier "
                                + "attribute but not all FlowFiles have a fragment.count attribute",
                        new Object[] { flowFile, flowFiles.size() });
            }
            ...
            try {
                ...
                throw illegal.apply(
                        "Cannot process %s because the fragment.count attribute has a value of '%s', which is not an integer",
                        new Object[] { flowFile, fragmentCount });
            }

            if (numFragments < 1) {
                throw illegal.apply(
                        "Cannot process %s because the fragment.count attribute has a value of '%s', which is not a positive integer",
                        new Object[] { flowFile, fragmentCount });
            }
            ...
            if (selectedNumFragments == 0) {
                ...
                throw illegal.apply(
                        "Cannot process %s because the fragment.count attribute has different values for different FlowFiles with the same fragment.identifier",
                        new Object[] { flowFile });
            }

            final String fragmentIndex = flowFile.getAttribute(FRAGMENT_INDEX_ATTR);
            if (fragmentIndex == null) {
                throw illegal.apply("Cannot process %s because the fragment.index attribute is missing",
                        new Object[] { flowFile });
            }
            ...
            try {
                ...
                throw illegal.apply(
                        "Cannot process %s because the fragment.index attribute has a value of '%s', which is not an integer",
                        new Object[] { flowFile, fragmentIndex });
            }

            if (idx < 0) {
                throw illegal.apply(
                        "Cannot process %s because the fragment.index attribute has a value of '%s', which is not a positive integer",
                        new Object[] { flowFile, fragmentIndex });
            }

            if (bitSet.get(idx)) {
                throw illegal.apply(
                        "Cannot process %s because it has the same value for the fragment.index attribute as another FlowFile with the same fragment.identifier",
                        new Object[] { flowFile });
            }
        ...
        if (transactionTimeoutMillis != null) {
            if (latestQueueTime > 0L && System.currentTimeMillis() - latestQueueTime > transactionTimeoutMillis) {
                throw illegal.apply(
                        "The transaction timeout has expired for the following FlowFiles; they will be routed to failure: %s",
                        new Object[] { flowFiles });
            }
        }
    ...
    private FlowFile addErrorAttributesToFlowFile(final ProcessSession session, FlowFile flowFile,
    ...
    /**
     * A FlowFileFilter that is responsible for ensuring that the FlowFiles returned
     * either belong
     * to the same "fragmented transaction" (i.e., 1 transaction whose information
     * is fragmented
    ...
    static class TransactionalFlowFileFilter implements FlowFileFilter {
        ...
        public FlowFileFilterResult filter(final FlowFile flowFile) {
            ...
            if (selectedId.equals(fragmentId)) {
                // fragment id's match. Find out if we have all of the necessary fragments or
                // not.
                final int numFragments;
                ...
                if (numSelected >= numFragments - 1) {
                    ...
                    // We still need more fragments for this transaction, so accept this one and
                    // continue.
                    numSelected++;
                    return FlowFileFilterResult.ACCEPT_AND_CONTINUE;
                }
     ...
     * or not those FlowFiles represent a "fragmented transaction" - that is, a
     * collection of FlowFiles
    ...
    /**
     * A simple, immutable data structure to hold a Prepared Statement and a List of
     * FlowFiles
    ...
    private static class StatementFlowFileEnclosure implements FlowFileGroup {
        ...
        public PreparedStatement getNewStatement(final Connection conn, final boolean obtainKeys) throws SQLException {
            if (obtainKeys) {
                // Create a new Prepared Statement, requesting that it return the generated
                // keys.
                PreparedStatement stmt = conn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS);

                if (stmt == null) {
                    // since we are passing Statement.RETURN_GENERATED_KEYS, calls to
                    // conn.prepareStatement will
                    // in some cases (at least for DerbyDB) return null.
                    // We will attempt to recompile the statement without the generated keys being
                    // returned.
     ...
     * Returns Object to avoid NoClassDefFoundError when DBCPService class is not
     * available
     */
     ...
     */
    private Connection getConnectionFromDBCPService(Object dbcpService, java.util.Map<String, String> attributes)
            throws Exception {
        if (dbcpService == null) {
            return null;
        }
    ...
    private static class ValidationResultData {
        ...
        private ValidationResultData(boolean valid, String operationId, Long environmentId, String environmentName,
                String errorMessage) {
            this.valid = valid;
    ...
    private ValidationResultData performEnvironmentValidation(ProcessContext context, FlowFile flowFile,
            ...
            String operationId,
            Map<String, EnvironmentConfig> configCache,
            Map<Object, String> urlCache) {
        ...
        // Step 2: Query environment configuration (dual-layer caching)
        // Layer 1: Check batch-local cache (fastest)
        EnvironmentConfig envConfig = configCache.get(operationId);

        if (envConfig == null) {
            // Layer 2: Check Caffeine cache (persistent across batches)
            envConfig = environmentConfigCache.getIfPresent(operationId);

            if (envConfig != null) {
                // Found in Caffeine cache - store in batch-local cache
                getLogger().debug("Environment config cache HIT (Caffeine) for operation_id: {}", operationId);
                configCache.put(operationId, envConfig);
            } else {
                // Layer 3: Query database
                getLogger().debug("Environment config cache MISS - querying database for operation_id: {}",
                        operationId);
                try {
                    envConfig = queryEnvironmentConfig(engineDbcp, operationId);
                    if (envConfig != null) {
                        // Store in both caches
                        environmentConfigCache.put(operationId, envConfig);
                        configCache.put(operationId, envConfig);
                    }
                } catch (SQLException e) {
            ...
            getLogger().debug("Environment config cache HIT (batch-local) for operation_id: {}", operationId);
        }
        ...
        // Step 3: Extract JDBC URLs from controller services (dual-layer caching)
        String sourceJdbcUrl;
        String targetJdbcUrl;
        try {
            // Source JDBC URL - Layer 1: Batch-local cache
            sourceJdbcUrl = urlCache.get(sourceDbcp);
            if (sourceJdbcUrl == null) {
                // Layer 2: Caffeine cache
                String cacheKey = "source_" + System.identityHashCode(sourceDbcp);
                sourceJdbcUrl = jdbcUrlCache.getIfPresent(cacheKey);

                if (sourceJdbcUrl != null) {
                    getLogger().debug("JDBC URL cache HIT (Caffeine) for source");
                    urlCache.put(sourceDbcp, sourceJdbcUrl);
                } else {
                    // Layer 3: Extract from service
                    getLogger().debug("JDBC URL cache MISS - extracting from source service");
                    sourceJdbcUrl = extractJdbcUrlFromService(sourceDbcp);
                    jdbcUrlCache.put(cacheKey, sourceJdbcUrl);
                    urlCache.put(sourceDbcp, sourceJdbcUrl);
                }
            } else {
                getLogger().debug("JDBC URL cache HIT (batch-local) for source");
            }

            // Target JDBC URL - Layer 1: Batch-local cache
            targetJdbcUrl = urlCache.get(targetDbcp);
            if (targetJdbcUrl == null) {
                // Layer 2: Caffeine cache
                String cacheKey = "target_" + System.identityHashCode(targetDbcp);
                targetJdbcUrl = jdbcUrlCache.getIfPresent(cacheKey);

                if (targetJdbcUrl != null) {
                    getLogger().debug("JDBC URL cache HIT (Caffeine) for target");
                    urlCache.put(targetDbcp, targetJdbcUrl);
                } else {
                    // Layer 3: Extract from service
                    getLogger().debug("JDBC URL cache MISS - extracting from target service");
                    targetJdbcUrl = extractJdbcUrlFromService(targetDbcp);
                    jdbcUrlCache.put(cacheKey, targetJdbcUrl);
                    urlCache.put(targetDbcp, targetJdbcUrl);
                }
            } else {
                getLogger().debug("JDBC URL cache HIT (batch-local) for target");
            }
        } catch (SQLException e) {
            return ValidationResultData.failure(operationId,
